...
- D2.7 elimina los reinicios automáticos del workspace. Cambiar de organización
  conserva la URL, cancela y descarta el cache del tenant anterior y remonta la
  ruta sólo cuando el alcance nuevo está listo. Un deploy o chunk obsoleto
  conserva la pantalla y muestra una única actualización manual; ningún evento
  de Service Worker, preload o ErrorBoundary recarga por decisión propia;
- D2.8 extiende ese contrato a la navegación de las tres superficies. Clientes,
  Pipeline, límites de plan e invitaciones ya cambian de ruta dentro de la SPA;
  aceptar una membresía refresca primero el tenant activo. El primer control de
...
- D5.38 convierte la tienda online en la puerta visual y verbal de Nerqia. El
  hero muestra una vitrina ecommerce realista con asset propio; un solo pedido
  continúa hacia Gestión y Finance en una banda compartida, y los tabs empiezan
  por Tienda sin dar a las otras dos capas jerarquías distintas ni datos
  duplicados;
...
| Storefront | Marca configurable aislada del SaaS; imágenes rotas degradan a fallbacks propios. El resultado de compra separa carga, acceso verificado y recuperación por email sin revelar si un número existe; pago y emails comparten la misma capacidad server-side. La comunicación transaccional distingue creación, pago y despacho y responde retries/dobles clicks como éxito deduplicado, no como otra entrega. Crawlers reciben HTML del comercio. Una red caída no se pinta como 404, catálogo vacío, pedido inexistente, tracking ausente ni carrito vencido. La medición externa nace apagada, se decide por tienda y no mezcla Pixel IDs entre merchants. El diseño se edita en borrador y sólo cambia la tienda pública mediante una publicación versionada y reversible. | Parcial D5.35 2026-09-04 | D5.30–D5.31 pasaron checkout/carrito 2/2 publicados. D5.32 separa pedido/pago, corrige PageView SPA y hace operativo el bootstrap TikTok; pasó 4/4 publicado. D5.33 hace visible y recuperable el seguimiento y pasó 2/2 publicado. D5.34 agrega preferencia de privacidad y transporte CSP mínimo; pasó 2/2 publicado con terceros interceptados. D5.35 agrega draft, preview privada sin efectos, publicación, historial y rollback. Faltan múltiples borradores, preview compartible, señal de proveedor real y compra sandbox/real. |
...
entraron a la cola prioritaria. El 2026-09-04 la búsqueda pública
`site:nerqia.app` mostró ambas homes y URL Inspection confirmó para las dos
**La URL está en Google / La página está indexada / HTTPS**. El sitemap sigue
Correcto con 71 páginas descubiertas; no hay acciones manuales ni problemas de
seguridad. Rendimiento todavía informa 0 impresiones/clics y sin consultas, por
lo que no se afirma alcance, posición ni tráfico.
...
el selector completo y el estado agotado en el detalle. Nerqia probó primero
tres quick choices: el deploy `ce81f427` quedó sin overflow ni logs, pero la
captura mostró una fila de 612 px y grandes vacíos bajo las cards simples. La
evidencia cambió la decisión. La card compra directo sólo un producto simple;
si hay variantes, muestra disponibilidad/agotadas y un CTA de 44 px hacia
“Elegir sabor/talle/variante” en una única PDP. “Desde” sólo usa el menor SKU
comprable. La regla de precio quedó compartida por PDP y carrito; la orden sigue
resolviendo precio y stock en servidor. La puerta final pasó 2.665 tests, tipos,
lint sin errores y build/PWA. `bfb1b0a7` quedó Ready: la card real de 9 opciones
bajó de 612 a 459,8 px, frente a 434,2 px de una simple; mostró 7 disponibles +
2 agotadas, 0 radios/quick-add ambiguo, ancho 1.284/1.284 y 0 logs. El E2E de
card + PDP pasó **4/4** en Chromium/Pixel 5 sin escrituras. Matriz D5.29 cerrada;
“Desde” queda en evidencia pura porque producción tiene 0 overrides distintos.

D5.30 corrige el cierre móvil antes de tocar pagos. Tiendanube estructura la
compra en datos, entrega y pago; Shopify mantiene el resumen mientras protege
esa misma secuencia. En Nerqia estaba fijo el `aside` completo: productos,
cupón, desglose, errores y confirmación podían tapar los propios campos del
checkout. Ahora el resumen queda en el documento y sólo persiste una barra
compacta de **Total + acción** con safe area y 48 px táctiles. Desktop conserva
su resumen lateral sticky. Un estado bloqueado nombra la causa en el CTA en vez
de apagarlo sin explicación. La prueba Playwright no envía una compra y, desde
este slice, intercepta visita/carrito/inicio de checkout para que el contrato
"read-only" sea real y no altere analítica. Puerta local: 2.665 tests/286
archivos, tipos, lint sin errores y build/PWA. `7e5c3a22` quedó Ready; el bundle
publicado pasó **2/2** en Chromium/Pixel 5 con resumen estático mobile, lateral
sticky desktop, barra fija ≤120 px, safe bottom, cero overflow y cero errores
de consola. La captura mantuvo visibles Entrega/Tus datos y no confirmó una
orden. Matriz D5.30 cerrada; compra sandbox/real sigue pendiente.

D5.31 aplica esa frontera al paso anterior. El carrito tenía un bloque `fixed`
de ancho completo con cotizador, promoción, envío, subtotal, total y CTA, tanto
en teléfono como en escritorio. Tiendanube confirma que cotizador y contador de
envío gratis pertenecen al carrito; Shopify exige líneas, cantidades,
descuentos y salida a checkout. Nerqia no retira capacidad: deja un único
resumen en el flujo mobile/lateral sticky desktop y fija sólo Total + Finalizar
en 360 px. Mientras el flete siga pendiente, muestra `importe + envío`; una
caída de cotización se ve y se registra. La guarda y el E2E read-only cubren
geometría, total pendiente, overflow y consola. La puerta local pasó TypeScript,
lint con 0 errores/143 warnings conocidos, 2.665 tests en 286 archivos y
build/PWA. `6f8e9f66` quedó Ready con alias raíz/wildcard y la matriz publicada
pasó **2/2** en Chromium/Pixel 5, sin overflow ni errores de consola; la
inspección visual desktop confirmó el resumen lateral sin contenido tapado.
D5.31 cerrado; compra sandbox/real sigue pendiente.

D5.32 corrige la medición que sostiene adquisición. El código decía soportar
TikTok, pero sólo descargaba un JavaScript genérico sin instalar la cola base ni
ejecutar `ttq.load(Pixel ID)`. También llamaba `CompletePayment` para un pedido
pendiente y duplicaba el PageView inicial, mientras GA4 perdía la navegación
SPA. El contrato ahora separa pedido colocado de pago acreditado, emite un solo
PageView por ruta, usa `transaction_id`/`event_id` estables y conserva un receipt
local sin PII para la recarga. Es la traducción de `checkout_completed` único en
Shopify, `transaction_id` de GA4 y `PlaceAnOrder`/`CompletePayment` de TikTok;
cinco pruebas focales protegen bootstrap, semántica y deduplicación. La puerta
local pasó TypeScript, lint con 0 errores/142 warnings conocidos, 2.670 tests en
287 archivos y build/PWA. `64ab5bd6` quedó Ready con alias raíz/wildcard y el
bundle publicado pasó **4/4** en Chromium/Pixel 5 recorriendo home y navegación
SPA por filtro/URL, sin errores de consola. La consulta agregada encontró una
tienda activa y **0 IDs Meta/GA4/TikTok configurados**: falta configurar un
píxel de prueba y observar la señal real del proveedor. No se declara Events
API/CAPI.

D5.33 cierra otro estado invisible del mismo postcompra. Shopify usa la página
de estado como destino principal y Tiendanube reúne allí orden, pago, preparación,
envío y código del transportista. Nerqia ya conserva una sola página y una
capacidad segura; no crea otra. El gap era visual y operativo: el tracking
tragaba rechazos y `{ error }` de Supabase y desaparecía. Ahora reserva espacio
durante carga, expone error sin borrar el pedido, ofrece reintento de 44 px,
invalida respuestas tardías y convierte un `found: false` incoherente en estado
recuperable. Tres pruebas de componente cubren carga, error/retry e inconsistencia.
La puerta local pasó TypeScript, lint con 0 errores/142 warnings conocidos,
**2.673 tests en 288 archivos** y build/PWA. La matriz publicada con RPC
interceptado pasó 2/2 en Chromium y Pixel 5 sin escribir pedidos.

D5.34 corrige el límite de medición encontrado en el header productivo. La CSP
impedía descargar los tres SDK; permitirlos globalmente sin otra condición
habría medido antes de que el comprador eligiera. El patrón propio suma un
banner compacto y no modal, dos decisiones de 44 px y acceso persistente desde
el footer. “Sólo esencial” deja intactos carrito/pedido y no carga terceros;
“Aceptar medición” habilita la allowlist mínima y direcciona cada evento al ID
del merchant activo. La decisión queda versionada por slug/origen y se sincroniza
entre pestañas. Ocho pruebas nuevas y trece del contrato cubren UI, storage,
corte, runtime, rutas con capacidades, tenant routing y CSP. La puerta completa
pasó TypeScript, lint con 0 errores/142 warnings conocidos, **2.685 tests en 290
archivos** y build/PWA. `a55f293f` llegó a Vercel `Ready`; el header canónico
respondió 200 con allowlist exacta y la matriz sintética de consentimiento pasó
2/2 en Chromium/Pixel 5 sin datos ni proveedores externos. Falta una señal real
autorizada; la tienda activa tiene cero IDs.

D5.35 convierte el editor de diseño en un flujo de release, no en un formulario
que escribe directamente sobre la tienda pública. La superficie conserva la
configuración publicada, permite guardar un borrador con nombre, abrirlo en una
preview autenticada del Storefront real, publicarlo con confirmación y volver a
cualquier versión anterior sin destruir historial. Guardar otras secciones de
la tienda ya no puede filtrar visuales del borrador abierto hacia producción.

La preview usa una ruta propia dentro del mismo Storefront y un RPC autorizado;
no crea un renderer paralelo. Muestra una franja persistente de contexto,
deshabilita checkout y omite visitas, tracking externo, hidratación y guardado
server-side del carrito. Carga, error, vacío, cambios sin guardar, conflicto
entre pestañas y confirmaciones forman parte del contrato. La verificación SQL
reversible pasó publicación/rollback/permisos y dejó cero restos; 16 pruebas
nuevas de dominio, guardas, componente y consentimiento llevan la suite a
...
D5.38 corrige la jerarquía comercial que D5.36 todavía dejaba incompleta. La
tienda online pasa al H1, primer enlace, CTA y primera tab; el centro de control
genérico se reemplaza por una vitrina ecommerce con fotografía de producto
propia. La escena no inventa otra aplicación: una banda continua toma el mismo
pedido y explicita su efecto en Gestión y Finance con igual peso secundario. El
texto SEO del borde y los metadatos comparten la misma promesa, y todo valor de
la demostración se rotula como ilustrativo.

El asset se generó sin marca, texto ni host de terceros y se optimizó de
...
- carrito y checkout: D5.30 evita el resumen completo sticky en checkout y
  pasó desktop/Pixel 5 publicado 2/2; D5.31 aplica la misma frontera al carrito,
  conserva el cotizador, declara `+ envío` y pasó publicado 2/2;
...
**Estado:** parcial; D6.1-D6.3 cerrados, 2026-09-04.
...
una causa transversal: naranja `#f59f0a` + blanco en 2,13:1 y el placeholder
del backoffice filtrado al checkout oscuro en 1,19:1. El tema ahora elige negro
o blanco mediante luminancia relativa WCAG, y los controles públicos consumen
únicamente tokens del merchant. La guarda axe cubre Inicio, PLP, PDP, carrito y
checkout en desktop/mobile y bloquea impactos críticos o serios. El primer
bundle corrigió 8/10 recorridos publicados; la PDP reveló dos links dentro de
texto distinguibles sólo por color/hover, ahora subrayados de forma permanente.
El segundo bundle quedó `Ready`, la ficha pasó 2/2 y la matriz completa cerró
**10/10 en Chromium/Pixel 5** sin escrituras productivas. Axe tampoco sustituye
teclado, zoom o lector de pantalla, por lo que D6 continúa parcial.

D6.2 comienza por el punto de entrada del teclado. La prueba publicada previa
falló porque el primer `Tab` no ofrecía salto al contenido: ahora el primer
control del DOM es un skip link que aparece con foco, apunta a un `<main>`
enfocable y usa un indicador blanco/negro independiente del tema del comercio.
La transición se anula con reducción de movimiento. El contrato sigue las
guías oficiales verificadas de
[Shopify](https://shopify.dev/docs/storefronts/themes/best-practices/accessibility)
y [W3C G1](https://www.w3.org/WAI/WCAG22/Techniques/general/G1.html), y queda
...
`4337dec7` quedó `Ready`, pero la matriz encontró dos composiciones reales más:
el input del buscador Sport era pisado a blanco sobre casi blanco (1,07:1) y la
transparencia del header Natural reducía 4,54:1 teórico a 4,16:1. El buscador
ahora toma fondo/texto de su superficie y el header usa el token opaco medido.
`ef6fab0e` quedó `Ready`: móvil pasó los siete temas y desktop detectó que
`opacity-80` degradaba los links Natural a 3,5:1. Se retiró la opacidad porque
hover/foco ya tienen indicador propio. `2c6d5d5c` y el endurecimiento del skip
link `1a00764a` quedaron `Ready`; la matriz publicada final cerró **14/14 en
Chromium/Pixel 5**: siete temas, skip link, Inicio, PLP, PDP, carrito y checkout,
sin escribir la configuración ni datos del comercio. D6.3 queda cerrado para
contraste automático de los temas actuales. Continúan pendientes la navegación
completa con teclado, zoom 200% y una validación con lector de pantalla.
...
| 22 | Accesibilidad AA | Parcial; D6.1-D6.3 cerrados 2026-09-04 | Gate axe WCAG A/AA para Inicio, PLP, PDP, carrito y checkout; D6.1 cerró 10/10 y D6.2 agregó skip link/destino/foco, con `200131e5` Ready y matriz 12/12. D6.3 encontró Pastel 3,45:1, links Bold 2,07:1, buscador Sport 1,07:1, header Natural 4,16:1 y navegación Natural con opacidad en 3,5:1; separa tinta/acento y corrige las composiciones reales. `2c6d5d5c` y `1a00764a` quedaron Ready y la matriz publicada final pasó 14/14 en Chromium/Pixel 5, sin escrituras. Faltan recorrido teclado completo, zoom y lector de pantalla. |
