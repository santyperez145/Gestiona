...
| Ventas/POS | Lista/Rendimiento; POS fullscreen deliberado. La cola offline ya habla en tickets —no líneas— y muestra unidades, monto, antigüedad, sincronización parcial y fallo persistente sin fingir que Gestiona capturó una tarjeta. El reintento conserva una identidad canónica y el servidor devuelve el ticket previo sin duplicar sus efectos. El drill Playwright aislado ya modela 2 tickets→offline→reconexión parcial con RPC interceptado y cleanup fail-safe. | Parcial D2.5/D3 2026-08-29 | Ejecutar el drill autenticado cuando exista entorno E2E, completar cobro teclado/touch y medir tarea. |
