...
publicada antes de cerrar D5.24.
...
| 20 | Storefront y operación Commerce | Parcial D5.24 2026-09-04 | D5.1 cubre resiliencia de banners, hero, categorías, cards, PDP, búsqueda, logo, carrito y sugerencias. D5.7 deja el CTA de compra al pie en 360 px. D5.8 reserva geometría al cargar y declara tamaño de imagen. D5.9 hace que crawlers vean HTML del comercio. D5.10 distingue 404 de red caída y no pinta un catálogo vacío. D5.11 no pide email ni declara el carrito vencido si la red falló. D5.12 conserva enlaces `/catalogo/:id` user/org. D5.13 hace visible y honesta la persistencia server-side y recupera todas las líneas/variantes. D5.14 reutiliza ese mismo Storefront en `<slug>.nerqia.app` con base, canonical, robots, sitemap, feed y previews resueltos por host. D5.15 corrige Inicio, H1 y robots a partir de la recorrida real. D5.20 pagina el PLP publicado con 20 cards, URL/retorno/filtros, 44 px y cero overflow/logs. D5.21 separa facturación paga, pedidos exactos y conversión atribuible mediante snapshot tenant-safe, declara la cobertura histórica, deja de contar carritos vencidos y quedó certificada publicada en 1.440/360 px sin overflow ni logs. D5.22 suma checkout iniciado sobre la misma sesión canónica, idempotente y sin PII; flujo publicado 360 px y panel cerraron sin overflow/logs, con señal exacta y limpieza en 0. D5.23 suma período URL, comparación anterior equivalente y controles de 44 px sobre el mismo snapshot; quedó certificado publicado en 360 px sin overflow/logs, con reload y limpieza correctos. D5.24 alinea humano/crawler: anchors secuenciales, canonical por página, 20 fichas rastreables, jerarquía `OnlineStore`/migas/ItemList y sitemap sin señales falsas; falta certificar deploy. Faltan compra sandbox/real, atribución por canal y cursor server-side cuando el volumen lo exija. |
