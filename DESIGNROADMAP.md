...
| Ventas/POS | Lista/Rendimiento; Ventas suma inspector lateral/fullscreen mobile de ticket con deep link, contexto, líneas, cobro, factura, devolución y margen registrado sin perder filtros; las acciones ya no dependen del hover. POS permanece fullscreen deliberado. La cola offline habla en tickets —no líneas— y el drill Playwright aislado modela 2 tickets→offline→reconexión parcial con RPC interceptado y cleanup fail-safe. El resumen de caja ya explica y persiste el mejor beneficio entre oferta/promoción y descuento configurado por medio; ticket y recibo muestran el precio realmente vendido. | Parcial D2.5/D3 2026-08-29 | Completar el workspace de cobro QR dinámico, validar POS/Ventas autenticado en cuatro viewports y ejecutar el drill offline cuando exista entorno E2E. |
...
| 13 | POS teclado/touch/offline | Parcial 2026-08-29 | F2/F9, objetivos táctiles y cola por ticket ya tienen contrato. La cola conserva monto/unidades/edad, auto-sync una vez, retry manual y error visible; una falla de almacenamiento bloquea otra venta offline antes de emitir un recibo falso. El estado pendiente está respaldado por identidad idempotente y prueba PostgreSQL reversible: 1 ticket/2 líneas/stock −3/2 deudas/cupón +1/0 restos al llamar v3 dos veces. Descuentos por efectivo/transferencia/débito/crédito ya compiten con oferta/promoción, se explican antes de cobrar y el servidor impide que un cliente obsoleto los borre; fixture real: ARS 10.000→9.000, pago/stock/evidencia correctos y 0 restos. El drill Playwright de 2 tickets/6 u./$9.500 y reconciliación parcial está implementado sin salida a producción; se listó/compiló, pero esta PC no tiene credenciales E2E. Falta QR dinámico acreditado antes del ticket, ejecutar el drill autenticado y medir tiempo/errores de cobro. |
