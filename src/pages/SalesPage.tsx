    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">{sales.length} ventas · Total: {formatARS(totalSales)} · Ganancia: {formatARS(totalProfit)}</p>
        </div>
...
function SaleForm({ userId, onSave }: { userId: string; onSave: () => void }) {
  ...
  const unitPrice = applyDiscount ? Number(product!.discount_price_ars) : (Number(product?.sale_price_ars) || 0);
        ...
        {product?.discount_price_ars && (
          <div><label className="text-sm text-muted-foreground">Descuento</label>
            <Select value={useDiscount} onValueChange={setUseDiscount}><SelectTrigger className="bg-muted border-border"><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="false">Normal</SelectItem><SelectItem value="true">Con descuento</SelectItem></SelectContent>
            </Select>
          </div>
        )}
      </div>
