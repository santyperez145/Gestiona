...
import { ArrowRight, ClipboardList, Gift, Mail, Plus, Target, Users, Wallet } from 'lucide-react';
import { useQuery } from '@tanstack/react-query';
import { useOrg } from '@/lib/orgContext';
import { useModulePerms } from '@/lib/permissionsContext';
import { isActiveInfluencer, listInfluencerInvitations, listInfluencers } from '@/lib/influencersDB';
...
const INVITATION_LABELS: Record<string, string> = { pending: 'Pendiente', accepted: 'Aceptada', declined: 'Rechazada', expired: 'Expirada' };
...
export default function InfluencerMarketingPage() {
  ...
  const invitations = useQuery({ queryKey: ['influencer-invitations', activeOrg?.id], queryFn: () => listInfluencerInvitations(), enabled: Boolean(activeOrg?.id) && canCreate, refetchOnWindowFocus: false, retry: false });
  ...
  const pendingInvites = (invitations.data ?? []).filter(item => item.status === 'pending');
  const metrics = [
  ...
  return <div className="space-y-7">
    ...
    {canCreate && (
      <section aria-labelledby="pending-invitations">
        <div className="mb-4 flex items-center justify-between gap-3"><h2 id="pending-invitations" className="text-base font-semibold">Invitaciones pendientes de respuesta</h2><Button asChild variant="ghost" size="sm"><Link to="/influencer-marketing/creadores">Invitar desde el directorio<ArrowRight className="ml-2 h-4 w-4" /></Link></Button></div>
        {invitations.isError ? null : !pendingInvites.length ? (
          <WorkspaceState kind="empty-first-use" title="Sin invitaciones pendientes" description="Invitá a un creador desde el directorio y verás su respuesta acá." />
        ) : <div className="divide-y divide-border">{pendingInvites.slice(0, 5).map(item => {
          const creator = (creators.data ?? []).find(c => c.id === item.influencer_id);
          return <div key={item.id} className="flex items-center gap-3 py-4"><Mail className="h-5 w-5 shrink-0 text-primary" /><div className="min-w-0 flex-1"><p className="break-words text-sm font-medium">{creator?.name ?? 'Creador'}</p><p className="mt-1 text-xs text-muted-foreground">Vence el {new Date(item.expires_at).toLocaleDateString('es-AR')}</p></div><Badge variant="outline">{INVITATION_LABELS[item.status] ?? item.status}</Badge></div>;
        })}</div>}
      </section>
    )}
