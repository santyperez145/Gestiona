...
export default function InfluencerRecords({ kind }: { kind: Kind }) {
  ...
  const [verifying, setVerifying] = useState<InfluencerDeliverable | null>(null);
  ...
  return <div className="space-y-5"><PageHeader icon={isContract ? FileText : Calendar} eyebrow="Nerqia · Influencers" title={title} actions={permissions.canCreate && <Button onClick={() => setEditing('new')}><Plus className="mr-2 h-4 w-4" />{isContract ? 'Nuevo contrato' : 'Nuevo entregable'}</Button>} />
    ...
    <Dialog open={Boolean(verifying)} onOpenChange={open => { if (!open && !busy) setVerifying(null); }}>{verifying && <PublicationProofForm deliverable={verifying} onSaved={() => { setVerifying(null); void client.invalidateQueries({ queryKey }); }} />}</Dialog>
  </div>;
}
...
const PUBLICATION_PLATFORMS = { instagram: 'Instagram', tiktok: 'TikTok', youtube: 'YouTube', otro: 'Otro' } as const;
const LICENSE_TYPES = { organico: 'Orgánico (sólo la publicación)', 'uso_campaña': 'Uso en campaña', paid_ampliado: 'Paid ampliado', cesion_total: 'Cesión total' } as const;
type PublicationPlatform = keyof typeof PUBLICATION_PLATFORMS;
type LicenseType = keyof typeof LICENSE_TYPES;

/** Verifica la publicación real del entregable con derechos de uso versionados. */
function PublicationProofForm({ deliverable, onSaved }: { deliverable: InfluencerDeliverable; onSaved: () => void }) {
  const [form, setForm] = useState<{ platform: PublicationPlatform; url: string; screenshot: string; license: LicenseType; expires: string; notes: string }>({ platform: 'instagram', url: '', screenshot: '', license: 'uso_campaña', expires: '', notes: '' });
  const [busy, setBusy] = useState(false);
  const lock = useRef(false);
  const [error, setError] = useState('');
  const licenciaConVencimiento = form.license === 'paid_ampliado' || form.license === 'cesion_total';
  const update = (key: keyof typeof form, value: string) => setForm(previous => ({ ...previous, [key]: value }));
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (lock.current) return;
    if (!form.url.startsWith('https://')) { setError('La URL de la publicación tiene que ser https://.'); return; }
    if (licenciaConVencimiento && !form.expires) { setError('La licencia pagada o cedida necesita fecha de vencimiento.'); return; }
    lock.current = true; setBusy(true); setError('');
    try {
      await registerPublicationProof({
        deliverable_id: deliverable.id,
        platform: form.platform,
        publication_url: form.url.trim(),
        screenshot_url: form.screenshot.trim() || null,
        license_type: form.license,
        license_expires_at: form.expires || null,
        license_notes: form.notes.trim() || null,
      });
      toast.success('Publicación verificada: queda registrada con su licencia');
      onSaved();
    } catch {
      setError('No pudimos verificar la publicación. Revisá el enlace, la licencia y los permisos.');
    } finally { lock.current = false; setBusy(false); }
  };
  return <DialogContent><DialogHeader><DialogTitle>Verificar publicación</DialogTitle><DialogDescription>Registrá la publicación real de {deliverable.influencer_name} con su captura y el tipo de licencia acordada. Cada verificación se guarda como un registro nuevo.</DialogDescription></DialogHeader>
    <form onSubmit={submit} className="space-y-4">
      <fieldset disabled={busy} className="space-y-3">
        <div className="space-y-1"><Label htmlFor="proof-platform">Plataforma</Label><Select value={form.platform} onValueChange={value => update('platform', value)}><SelectTrigger id="proof-platform"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(PUBLICATION_PLATFORMS).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
        <div className="space-y-1"><Label htmlFor="proof-url">URL de la publicación</Label><Input id="proof-url" type="url" pattern="https://.*" required value={form.url} onChange={event => update('url', event.target.value)} placeholder="https://instagram.com/reel/..." /></div>
        <div className="space-y-1"><Label htmlFor="proof-screenshot">Captura (URL, opcional)</Label><Input id="proof-screenshot" type="url" pattern="https://.*" value={form.screenshot} onChange={event => update('screenshot', event.target.value)} placeholder="https://..." /></div>
        <div className="space-y-1"><Label htmlFor="proof-license" aria-label="tipo de licencia">Tipo de licencia</Label><Select value={form.license} onValueChange={value => update('license', value)}><SelectTrigger id="proof-license"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(LICENSE_TYPES).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
        {licenciaConVencimiento && <div className="space-y-1"><Label htmlFor="proof-expires">Licencia vigente hasta</Label><Input id="proof-expires" type="date" required min={new Date().toISOString().slice(0, 10)} value={form.expires} onChange={event => update('expires', event.target.value)} /></div>}
        <div className="space-y-1"><Label htmlFor="proof-notes">Notas de la licencia (opcional)</Label><Textarea id="proof-notes" maxLength={2000} value={form.notes} onChange={event => update('notes', event.target.value)} /></div>
      </fieldset>
      {error && <p role="alert" className="text-sm text-destructive">{error}</p>}
      <DialogFooter><Button type="button" variant="outline" disabled={busy} onClick={onSaved}>Volver</Button><Button type="submit" disabled={busy}>{busy ? 'Verificando...' : 'Registrar verificación'}</Button></DialogFooter>
    </form>
  </DialogContent>;
}
