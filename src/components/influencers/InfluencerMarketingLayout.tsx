...
const CORE_BRIDGES = [
  { to: '/marketing', label: 'Marketing de tienda', icon: Sparkles },
  { to: '/productos', label: 'Catálogo', icon: Gift },
];
  ...
  return (
    <div className="influencer-shell min-h-screen bg-card text-foreground lg:grid lg:grid-cols-[240px_minmax(0,1fr)]">
      <aside className="influencer-sidebar border-b border-border lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
        ...
        <div className="influencer-nav flex items-center gap-1 overflow-x-auto p-3 lg:block lg:space-y-0.5 lg:p-3">
          {NAV.map(item => (
            <NavLink
              key={item.to}
              to={item.to}
      ...
      <main className="min-w-0 bg-background">
        <header className="influencer-topbar sticky top-0 z-20 hidden h-14 items-center justify-between border-b border-border bg-background/94 px-6 backdrop-blur lg:flex">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-amber-700 dark:text-amber-300">Influencer Marketing</p>
