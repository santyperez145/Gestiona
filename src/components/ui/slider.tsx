  ...
  ({ className, min = 0, max = 100, step = 1, value, onValueChange, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          "relative h-2 w-full rounded-full bg-muted/10",
          "after:content-[''] after:absolute after:inset-0 after:rounded-full after:bg-muted/20",
      ...
      >
        <div
          className={cn(
            "absolute left-1 top-1/2 -translate-y-1/2 rounded-full h-3.5 w-3.5 bg-primary transition-all duration-200 ease-out",
        ...
        >
          <div
            className={cn(
              ...
              "data-[state=active]:translate-y-1 data-[state=active]:bg-primary",
              className,
            )}
          />
        </div>
