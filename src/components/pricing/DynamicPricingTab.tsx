/**
 * DynamicPricingTab — ported from the former DynamicPricingPage (/precios-dinamicos, deleted).
 * Rendered as the "Dinámico" tab inside PricingEnginePage (/motor-precios).
 *
 * Rules that auto-adjust prices by time/day/demand/stock/competitor + demand signals
 * feeding suggested prices + a history of applied price events + a what-if simulator.
 */
...
import {
  Zap, TrendingUp, TrendingDown, BarChart3, Brain,
  Plus, Play, DollarSign,
} from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">Reglas automáticas de precios por tiempo, demanda, stock e IA</p>
        <Dialog open={showNew} onOpenChange={setShowNew}>
          <DialogTrigger asChild><Button size="sm"><Plus className="w-4 h-4 mr-2" />Nueva Regla</Button></DialogTrigger>
          <DialogContent>
      ...
      <Tabs value={tab} onValueChange={v => setTab(v as typeof tab)}>
        ...
        {/* RULES */}
        <TabsContent value="rules" className="space-y-3 pb-4">
          ...
          {rules.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-8">Sin reglas dinámicas configuradas</p>
          )}
        </TabsContent>

        {/* DEMAND */}
        <TabsContent value="demand" className="space-y-3 pb-4">
          ...
          {demandSignals.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-8">Sin señales de demanda recientes</p>
          )}
        </TabsContent>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {priceEvents.length === 0 && (
                    <tr><td colSpan={7} className="text-center py-10 text-muted-foreground">Sin eventos de ajuste registrados</td></tr>
                  )}
                </tbody>
              </table>
            </CardContent>
