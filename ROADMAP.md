...
| Rediseño público v4 | La landing de Nerqia fue reconstruida el 2026-09-04 como portada de producto. D5.38 fija la jerarquía comercial definitiva: el H1 y el CTA abren por **Tienda online**; una vitrina ecommerce con fotografía propia reemplaza el dashboard genérico y el mismo pedido muestra, en una banda continua, qué sincroniza Gestión y qué explica Finance. Las tres superficies conservan una única autoridad y Gestión/Finance tienen peso equivalente detrás de la venta. El asset WebP local pesa 82.936 bytes, el E2E pasó 12/12 en Chromium/mobile a 360/768/1024/1280×720/1440 y Axe no encontró impactos críticos/serios. `05797fca` quedó `Ready` con los alias de `nerqia.app`; una pestaña productiva nueva confirmó a 1.280×720 la siguiente sección desde 643 px, ancho 1.276/1.280, imagen y flujo completos, Tienda activa y cero logs `warn`/`error`. D5.37 separa los shells privados sin fragmentar el diseño: el HTML público bajó de 26 a 11 assets iniciales y de 342.897 a 295.641 bytes gzip. D5.1 mantiene la resiliencia transversal de medios del Storefront. | Medir conversión real del CTA y después retirar el CSS v3 muerto sin cambiar la dirección visual. |
...
194. Nerqia se presenta como producto, no como una plantilla — D5.36,
     2026-09-04. La landing deja el hero dividido y el titular genérico: la
     primera señal ahora es **Nerqia, el sistema operativo de tu comercio**, en
     una superficie frambuesa full-bleed con un centro de control claro que
     muestra la operación real del producto. La narrativa se reduce a Business
     Core, Commerce y Finance; los tres comparten autoridad y las tabs explican
     sus límites sin inventar otro stock, cliente o venta. Se retiró la cita con
     iniciales que podía leerse como testimonio y todas las cifras del preview
     declaran ser ilustrativas.

     El primer E2E encontró dos defectos que la captura desktop no mostraba:
     el hero excedía el pliegue por 95 px a 360 y 18 px a 768, y varios textos
     pequeños no alcanzaban contraste AA. Se corrigieron sin aflojar la prueba.
     La matriz final pasó **5/5 casos Playwright** en 360/768/1024/1440, tabs
     por teclado, menú mobile, continuidad y cero overflow; Axe quedó sin
     impactos críticos o serios y localhost no emitió errores propios. La
     puerta pasó typecheck, lint con **0 errores/142 warnings conocidos**,
     **2.714 tests en 295 archivos** (`npm test`, 2026-09-04) y build/PWA. No
     suma dependencias, imágenes ni requests: el build medido el 2026-09-04
     queda en **119,99 kB gzip de JS inicial y 49,67 kB gzip de CSS**; el costo
     frente al build anterior fue +0,89 kB y +2,32 kB respectivamente. Falta
     medir conversión real y retirar el CSS v3 que la cascada ya no usa. El
...
196. Un mismo diseño no obliga a descargar tres shells — D5.37, 2026-09-04.
     El análisis del `dist/index.html` mostró que la landing precargaba 26
     assets por **342.897 bytes gzip**. AppLayout, permisos, MFA y los layouts
     de Platform y Finance entraban en el árbol estático aunque el visitante no
     hubiera abierto ninguna superficie privada. Esos siete límites pasan a
     `lazy()` bajo el Suspense raíz; la arquitectura y el sistema visual siguen
     siendo únicos, pero cada persona paga el chrome que usa.

     El mismo cálculo queda como guarda opcional sobre el build: máximo 12
     recursos y 310.000 bytes gzip, sin App/Platform/Finance Layout en el HTML
     inicial. El resultado medido es **11 assets / 295.641 bytes gzip**, 47.256
     bytes menos (**-13,8%**); el entry JS baja de 119.083 a 90.882 bytes gzip
     (**-23,7%**). La matriz de landing volvió a pasar **5/5** en
     360/768/1024/1440 y Axe sin impactos críticos/serios. La puerta serial
     pasó typecheck, lint con **0 errores/142 warnings conocidos**, **2.718
     tests en 296 archivos** (`npm test`, 2026-09-04) y build/PWA. Supabase,
     Query y primitives UI aún son comunes: se separan sólo con otra medición y
     sin romper sesión, SEO ni navegación.

197. La tienda online pasa a ser la puerta visible de Nerqia — D5.38,
     2026-09-04. La landing todavía abría con un centro de control y dejaba la
     tienda como una tab secundaria. El H1, la navegación y el CTA ahora parten
     de **Tienda online**; una vitrina de demostración con fotografía propia
     muestra marca, catálogo y compra, y una banda única continúa el pedido en
     Gestión —stock reservado— y Finance —margen explicado—. No crea tres
     productos ni tres fuentes de verdad: comunica tres superficies sobre el
     mismo Business Core. El SEO visible, el HTML semántico del borde y los
     metadatos sociales usan la misma promesa.

     El PNG generado para la escena medía 1.876.857 bytes; su versión WebP local
     de 1.600 px pesa **82.936 bytes** y no depende de un host externo. El primer
     E2E encontró que el hero excedía el pliegue entre 10 y 37 px según ancho y
     que el rótulo Finance quedaba en 4,43:1 de contraste. Se compactó la escena
     conservando sus tres señales y se elevó el contraste, sin aflojar la
     prueba. La inspección del primer deploy encontró un quinto caso que la
     matriz no cubría: a 1.280×720 la sección siguiente quedaba fuera del
     pliegue. El mockup ahora se escala completo —sin cortar ninguna capa— y ese
     viewport queda como regresión permanente. La matriz final pasó **12/12** en
     Chromium/mobile a 360/768/1024/1280×720/1440, sin overflow y con Axe WCAG
     A/AA sin impactos críticos o serios. Se retiró además una prop de
     prioridad no soportada que React reportaba en consola. Puerta completa:
     typecheck, lint con **0 errores/142 warnings conocidos**, **2.719 tests en
     296 archivos** (`npm test`, 2026-09-04) y build/PWA; CSS en **49,39 kB
     gzip**. `05797fca` quedó `Ready` en Vercel con alias `nerqia.app`, `www` y
     wildcard. Una pestaña productiva nueva confirmó a 1.280×720 la sección
