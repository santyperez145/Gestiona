...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "sync") {
      ...
      if (!batch.mp_payout_id) return json({ error: "El lote todavía no fue enviado a Mercado Pago" }, 400);

      const creds = await getMpCredentials(admin, batch.org_id);
      if (!creds) return json({ error: "Mercado Pago no está conectado" }, 400);

      const res = await fetch(
        `https://api.mercadopago.com/v1/payouts/${encodeURIComponent(batch.mp_payout_id)}/transactions`,
        {
      ...
      if (!res.ok) return json({ error: extractErrorMessage(payload) }, res.status);
      ...
      return json({ ok: true, status: batchStatus, aprobados, rechazados });
    }
