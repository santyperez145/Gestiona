 ...
 *
 * Sólo se acepta el manifiesto oficial. Aceptar formatos alternativos amplía
 * innecesariamente la superficie de verificación de un endpoint que mueve
 * dinero.
 */
async function verifyMpSignature(
  ...
  try {
    ...
    const received = new Uint8Array(v1.match(/.{2}/g)!.map((byte) => Number.parseInt(byte, 16)));
    const manifest = `id:${paymentId};request-id:${requestId};ts:${ts};`;
    return await crypto.subtle.verify("HMAC", key, received, enc.encode(manifest));
  } catch {
    return false;
  }
}

function signedResourceId(req: Request, bodyId: string): string | null {
  const urlId = new URL(req.url).searchParams.get("data.id")?.trim() ?? "";
  const normalizedBodyId = bodyId.trim();
  if (urlId && normalizedBodyId && urlId !== normalizedBodyId) return null;
  const resourceId = (urlId || normalizedBodyId).toLowerCase();
  return resourceId && /^[a-z0-9_-]+$/.test(resourceId) ? resourceId : null;
}

async function requireValidSignature(req: Request, bodyId: string): Promise<string | Response> {
  const resourceId = signedResourceId(req, bodyId);
  const signature = req.headers.get("x-signature") || "";
  const requestId = req.headers.get("x-request-id") || "";
  const secret = Deno.env.get("MP_WEBHOOK_SECRET") || "";

  if (!secret) {
    console.error("MP_WEBHOOK_SECRET no está configurado; webhook bloqueado");
    return new Response(JSON.stringify({ ok: false, reason: "webhook not configured" }), {
      status: 503,
      headers: { "Content-Type": "application/json" },
    });
  }
  if (!resourceId || !signature || !requestId
    || !await verifyMpSignature(resourceId, requestId, signature, secret)) {
    console.warn("Mercado Pago webhook rechazado: firma o recurso inválido");
...
async function reconcilePendingStoreRefunds(
  ...
  try {
    ...
    throw new Error("Mercado Pago no respondió al reconciliar reintegros", { cause: error });
  }

  if (!response.ok) {
    throw new Error(`Mercado Pago rechazó la consulta de reintegros (${response.status})`);
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (type === "subscription_preapproval" || type === "subscription_authorized_payment") {
      ...
      const verifiedId = await requireValidSignature(req, suscId);
      if (verifiedId instanceof Response) return verifiedId;

      const platformToken = await tokenDeLaPlataforma();

      if (!platformToken) {
        console.error("Webhook de suscripción sin token de plataforma");
        return new Response(JSON.stringify({ ok: false, reason: "provider not configured" }), {
          status: 503, headers: { "Content-Type": "application/json" },
        });
      }

      const ruta = type === "subscription_preapproval"
        ? `https://api.mercadopago.com/preapproval/${verifiedId}`
        : `https://api.mercadopago.com/authorized_payments/${verifiedId}`;
      ...
      if (!rsp.ok) {
        console.error(`No se pudo revalidar ${type}: ${rsp.status}`);
        return new Response(JSON.stringify({ ok: false, reason: "provider validation failed" }), {
    ...
    if (["order", "orders"].includes(type) && paymentId) {
      const verifiedId = await requireValidSignature(req, paymentId);
      if (verifiedId instanceof Response) return verifiedId;

      const { data: session, error: sessionError } = await admin
        .from("pos_qr_sessions")
        .select("id, org_id")
        .eq("provider_order_id", verifiedId)
        .maybeSingle();
      ...
      if (!credentials) {
        console.error("Sin OAuth para reconciliar una order de Mercado Pago");
        return new Response(JSON.stringify({ ok: false, reason: "provider not connected" }), {
          status: 503, headers: { "Content-Type": "application/json" },
        });
      }
      const order = await fetchMercadoPagoOrder(credentials.accessToken, verifiedId);
      if (String(order.id ?? "").toLowerCase() !== verifiedId) {
        return new Response(JSON.stringify({ ok: false, reason: "provider resource mismatch" }), {
          status: 502, headers: { "Content-Type": "application/json" },
        });
      }
      ...
      console.log(`MP POS QR order conciliada: ${String(reconciled.state ?? "unknown")}`);
      return new Response(JSON.stringify({ ok: true, orderId: verifiedId, state: reconciled.state }), {
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    // `org_id` es sólo una pista de enrutamiento. La pertenencia real se
    // demuestra al reconsultar el pago con el OAuth privado del comercio.
    const url = new URL(req.url);
    const orgIdFromQuery = url.searchParams.get("org_id") || "";
    if (!orgIdFromQuery) {
      return new Response(JSON.stringify({ ok: false, reason: "missing tenant routing" }), {
        status: 400, headers: { "Content-Type": "application/json" },
      });
    }
    ...
    // El body nunca es autoridad para el estado ni el importe.
    const mpRes = await fetch(`https://api.mercadopago.com/v1/payments/${verifiedId}`, {
      headers: { Authorization: `Bearer ${mpAccessToken}` },
    });

    if (!mpRes.ok) {
      console.error(`Mercado Pago payment revalidation failed: ${mpRes.status}`);
      return new Response(JSON.stringify({ ok: false, reason: "provider validation failed" }), {
        status: 502, headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (String(payment.id ?? "") !== verifiedId) {
      return new Response(JSON.stringify({ ok: false, reason: "provider resource mismatch" }), {
        status: 502, headers: { "Content-Type": "application/json" },
      });
    }
        ...
        if (!saleErr) {
          ...
          if (membership?.user_id) {
            try {
              ...
              console.error("mercadopago-webhook notification lookup:", notificationError);
            }
          }
        }
      }
    ...
    // Los RPC son idempotentes: un error interno debe provocar reintento del
    // proveedor, no una falsa confirmación 200 que pierda el evento.
