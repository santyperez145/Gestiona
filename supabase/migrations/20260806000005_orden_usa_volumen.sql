-- create_store_order pasa a usar el descuento por volumen.
--
-- `store_volume_discount` toma por producto el **mejor** entre el precio 2x fijo
-- y la mejor regla de `quantity_discounts`, en vez de mirar sólo el 2x.
--
-- La variable interna sigue llamándose `v_promo_2x` y el campo de la respuesta
-- también: renombrarlos obligaría a tocar el cuerpo entero y el cliente por un
-- nombre, y el número que llevan es el mismo concepto —lo que se ahorra por
-- llevar varios—. El comentario de arriba de la línea dice qué calcula.
--
-- Regenerada desde la definición que corre en producción con una sola línea
-- cambiada. Idempotente.
