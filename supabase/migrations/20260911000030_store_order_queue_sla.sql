-- C23: SLA configurable de preparación de pedidos por tienda (Shopify: processing time).
-- Antes el umbral de atrasados estaba hardcodeado a 24 h en el SQL y en el cliente.
-- Cada tienda puede configurar su tiempo de preparación (12h, 24h, 48h, 72h, 120h).
BEGIN;

ALTER TABLE public.ecommerce_stores
  ADD COLUMN IF NOT EXISTS fulfillment_sla_hours integer NOT NULL DEFAULT 24;

COMMENT ON COLUMN public.ecommerce_stores.fulfillment_sla_hours IS
  'Horas objetivo para despachar o poner a retiro un pedido pagado. Alimenta la vista Atrasados.';
...
BEGIN
  ...
  SELECT coalesce(nullif(fulfillment_sla_hours, 0), 24) INTO v_sla_hours
  FROM public.ecommerce_stores
  WHERE id = p_store_id AND org_id = p_org_id;
  ...
  WITH scoped AS MATERIALIZED (
    ...
    SELECT *, ARRAY['todas']::text[]
      ...
      || CASE WHEN awaiting AND created_at < now() - make_interval(hours => v_sla_hours) THEN ARRAY['atrasados'] ELSE '{}'::text[] END
