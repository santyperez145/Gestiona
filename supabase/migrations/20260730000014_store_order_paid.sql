-- Confirmación de pago de una orden de la tienda online.
--
-- Hasta acá la orden quedaba en `pending` y nadie descontaba stock ni la
-- registraba como venta: la facturación online no aparecía en el Dashboard,
-- ni en Reportes, ni en el P&L. Una tienda que no impacta en los números del
-- negocio no sirve de mucho.
--
-- Esta función hace las tres cosas de forma atómica e idempotente: MercadoLibre
-- y MercadoPago reintentan sus webhooks, así que cobrar dos veces el mismo
-- pedido tiene que ser imposible.
...
BEGIN
  ...
  SET payment_status     = 'paid',
      ...
      tracking_number    = COALESCE(tracking_number, p_payment_id),
      updated_at         = now()
  WHERE id = p_order_id;
