-- ═══════════════════════════════════════════════════════════════════════════
-- El asiento de una orden online salia SIN costo de mercaderia
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Encontrado extendiendo la traza de P0-07, y confirmado con una asercion nueva
-- en la matriz de pagos: **el asiento contable de una orden de la tienda tenia
-- la cuenta 5.1.01 (Costo de mercaderia vendida) en cero.**
--
-- La causa: `ledger_asentar_orden_pagada` buscaba los movimientos de stock con
-- `reference_id = <orden>`, y el movimiento no referencia la orden — referencia
-- la **venta** que la orden generó. Medido: los 34 movimientos de stock que hay
-- en produccion tienen `reference_type = 'sale'`; **ninguno** apunta a una
-- orden.
--
-- ── Por que no se veia ─────────────────────────────────────────────────────
--
-- Porque la funcion no falla cuando no encuentra movimientos: emite
-- `RAISE WARNING 'la orden % no tiene movimientos de stock'` y asienta igual,
-- con costo cero. Es la decision correcta —que falte el costo no puede hacer
-- fallar un cobro— pero convierte el problema en un warning que nadie lee.
--
-- El efecto es el peor posible para un numero de negocio: **el resultado del
-- periodo sale mejor de lo que es**. Ingresos menos gastos sin el costo de la
-- mercaderia no es un margen incompleto, es un margen equivocado y optimista.
--
-- Hoy no corrompio ningun dato: hay 0 asientos de tipo 'orden' en produccion
-- (medido 2026-08-25), porque ninguna orden online llego a generar venta con
-- `ecommerce_order_id`. Se arregla antes de que la primera lo haga.
--
-- La matriz de pagos ahora **exige** que 5.1.01 sea mayor que cero en el asiento
-- de la orden: sin esa asercion, este bug volveria sin que nada lo diga.
--
-- Regenerada desde `pg_get_functiondef` con un script, cambiando solo el WHERE.
--
-- Idempotente.
-- ═══════════════════════════════════════════════════════════════════════════
