-- El descuento por medio de pago dejaba de acumularse con la oferta.
--
-- El dueño lo reportó mirando su propia tienda: un vaper con precio de lista
-- 38.640 y oferta 30.912 (20% off) mostraba "24.730 con transferencia (20%
-- OFF)". Son **36% de descuento real**, no 20: el porcentaje del medio de pago
-- se estaba aplicando sobre un subtotal que ya venía con el precio de oferta.
--
-- Y el precio tachado tampoco cerraba: decía 38.640 al lado de un precio final
-- que no era 38.640 menos ningún porcentaje redondo.
--
-- ── La regla ─────────────────────────────────────────────────────────────
--
-- **Los dos descuentos no se acumulan: se cobra el mejor, nunca la suma.**
--
-- El descuento del medio de pago se calcula por línea contra el precio de
-- **lista**: es lo que falta para llegar a `lista × (1 - pct)`. Si la oferta ya
-- deja el precio por debajo, no descuenta nada más.
--
...
-- si la tienda publica "20% OFF con transferencia", cobrar el 10% de la oferta
-- sería romper esa promesa.
--
-- Para poder compararlo, la línea que arma `resolve_store_line` ahora lleva
-- `list_price`. Con una variante que tiene `price_override`, su lista es ese
-- precio propio: la oferta del padre no le aplica.
--
-- Idempotente. Las dos funciones se regeneraron desde la definición que corre en
-- producción, con los cambios insertados por script.
...
BEGIN
  ...
  IF p_variant_id IS NULL THEN
    ...
    RETURN jsonb_build_object(
      ...
      'line', jsonb_build_object(
        ...
        -- El precio de lista viaja en la línea para poder calcular el descuento
        -- por medio de pago sobre él y no sobre el de oferta, que ya lo tiene.
        'list_price', v_prod.sale_price_ars,
  ...
  RETURN jsonb_build_object(
    ...
    'line', jsonb_build_object(
      ...
      'brand', v_prod.brand,
      'quantity', p_qty, 'unit_price', v_unit,
      -- Con `price_override` la variante tiene precio propio y ése es su lista:
      -- el de oferta del padre no aplica.
      'list_price', CASE WHEN COALESCE(v_var.price_override, 0) > 0
                         THEN v_var.price_override
